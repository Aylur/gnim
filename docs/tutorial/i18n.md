...
Instead, mark the position of the value with a
<span v-pre>`{{placeholder}}`</span> and fill it in with `fmt`.
...
Placeholder values can be strings or numbers. When only placeholders are filled
in, `fmt` returns a plain string.

The strings returned by `gettext`, `ngettext` and `pgettext` carry their
placeholders at the type level, so `fmt` is fully type checked: forgetting a
placeholder or misspelling its name is a compile-time error.

> [!NOTE]
>
> Translators have to preserve placeholders verbatim. A placeholder missing from
> the values object is left in the text as-is.
...
Placeholders and tags can be combined in the same message.
...
> [!NOTE]
>
> The markup parser is intentionally forgiving: tag names are case-insensitive,
> unclosed tags are closed automatically, and a tag without a matching function
> renders just its content.
...
its text. For these cases use `sfmt`: it substitutes placeholders exactly like
...
Like `fmt`, `sfmt` type checks its placeholders, but tags in the message require
no values: they are passed through to the output verbatim for Pango to
interpret.
